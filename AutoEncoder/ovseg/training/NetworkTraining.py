...
class NetworkTraining(TrainingBase):
    ...
    def __init__(self, network, trn_dl,  model_path, loss_params={},
                 ...
                 no_bias_weight_decay=False, save_additional_weights_after_epochs=[],dist_flag=False):
        ...
        print("NetworkTraining initialised loss complete")
        # setup optimizer
        self.initialise_opt()
    ...
    def do_trn_step(self, batch, step):
        ...
        if self.fp32:
            loss = self.compute_batch_loss(batch,self.dist_flag)
            loss.backward()
            self.opt.step()
        else:
            # with amp.autocast():
