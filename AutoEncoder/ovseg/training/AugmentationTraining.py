...
from ovseg.training.loss_functions_combined import CE_dice_pyramid_loss, to_one_hot_encoding, \
    weighted_combined_pyramid_loss, weighted_combined_loss
import torch
...
class AugmentationTraining(NetworkTraining):
    ...
    def __init__(self, *args,
                 ...
                 hd_weight= None,
                 **kwargs):
        super().__init__(*args, **kwargs,dist_flag=dist_flag)
    ...
    def initialise_loss(self):
        ...
        self.loss_fctn_normal = weighted_combined_pyramid_loss(**self.loss_params)
    ...
    def compute_batch_loss(self, batch):
        ...
        # the easiest one:
        xb = batch[:, 0:1].float().to(self.dev)
        yb = xb
        mask = None
        ...
        out = self.network(xb)
        normal_loss = self.loss_fctn_normal(out, yb, mask=mask)
...
class AugmentationTrainingV2(AugmentationTraining):
