...
class AutoEncoderBatchDataset(object):
    ...
    def _maybe_store_data_in_ram(self):
        ...
        if self.store_data_in_ram:
            ...
            for ind in tqdm(range(self.n_volumes)):
                ...
                labels = np.load(path_dict[self.image_key]).astype(self.dtype)
        ...
        if self.store_coords_in_ram:
            ...
            for ind in tqdm(range(self.n_volumes)):
                if self.store_data_in_ram:
                    labels = np.load(self.vol_ds.path_dicts[ind][self.image_key])
                else:
                    labels = np.load(self.vol_ds.path_dicts[ind][self.image_key])
    ...
    def _get_volume_tuple(self, ind=None):
        ...
        if load_from_ram:
            ...
            labels = np.load(path_dict[self.image_key], 'r')
            volumes = [im, labels]
...
def AutoEncoderDataloader(vol_ds, patch_size, batch_size, num_workers=None,
                           pin_memory=True, epoch_len=250, *args, **kwargs):
    dataset = AutoEncoderBatchDataset(vol_ds=vol_ds, patch_size=patch_size, batch_size=batch_size,
